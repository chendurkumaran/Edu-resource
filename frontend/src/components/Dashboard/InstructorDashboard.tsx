...
import type { DashboardData, CoursePerformance, Course } from '../../types'; // Import types
...
const InstructorDashboard = () => {
  ...
  const fetchCoursePerformance = async () => {
    ...
    try {
      ...
      const coursesResponse = await axios.get<Course[]>(`/api/courses/instructor/${user._id}`);
      const courses = coursesResponse.data;
      
      // Then fetch performance data for each course
      const performancePromises = courses.map(async (course) => {
        try {
          const perfResponse = await axios.get<CoursePerformance>(`/api/courses/${course._id}/performance`);
