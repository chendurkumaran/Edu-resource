...
import type { Course, AttendanceRecord, StudentCourseAttendance } from '../../types';
...
const AttendanceView = () => {
  const { user } = useAuth();
  const [attendanceData, setAttendanceData] = useState<StudentCourseAttendance[] | AttendanceRecord[]>([]);
  ...
  const fetchEnrolledStudents = async () => {
    try {
      ...
      const students = response.data.map((enrollment: any) => ({
  ...
  const handleStudentStatusChange = (studentId: string, status: string) => {
    setAttendanceForm(prev => ({
      ...
      students: prev.students.map(student =>
        student.student === studentId ? { ...student, status: status as EnrolledStudent['status'] } : student
      )
    }));
  };
      ...
      {user?.role === 'student' ? (
        ...
        <div className="space-y-6">
          {attendanceData.length === 0 ? (
            ...
            (attendanceData as StudentCourseAttendance[]).map((courseData) => (
