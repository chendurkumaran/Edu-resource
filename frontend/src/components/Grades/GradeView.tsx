...
import type { StudentGradeReport, Course } from '../../types';

const GradeView = () => {
  const { user } = useAuth();
  const [grades, setGrades] = useState<StudentGradeReport[]>([]);
  ...
  const fetchStudentGrades = async () => {
    if (!user) return;
    try {
      const response = await axios.get<StudentGradeReport[]>(`/api/grades/student/${user._id}`);
  ...
  const fetchInstructorCourses = async () => {
    if (!user) return;
    try {
      const response = await axios.get<Course[]>(`/api/courses/instructor/${user._id}`);
  ...
  const fetchCourseGrades = async () => {
    try {
      const response = await axios.get<StudentGradeReport[]>(`/api/grades/course/${selectedCourse}`);
      setGrades(response.data);
    } catch (error) {
  ...
  const calculateGPA = (grades: StudentGradeReport[]) => {
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {grades.map((grade) => (
                      <tr key={grade._id}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className={`px-2 py-1 rounded-full text-sm font-medium ${getGradeColor(grade.percentage || 0)}`}>
                            {grade.letterGrade}
                          </span>
                        </td>
